<?php
if (!isset($db)) $db = Database::getInstance();
$menus = get_menus();
$site_name = get_setting('site_name', 'Reklamenesia');
$wa_number = get_setting('wa_number');
$wa_link   = $wa_number ? wa_url($wa_number, get_setting('wa_text', 'Halo, saya ingin konsultasi.')) : url('/hubungi-kami');
$nav_layanan = $db->fetchAll("SELECT slug, nama FROM layanan WHERE is_active = 1 ORDER BY urutan ASC");
?>
<div class="m-overlay" id="m-overlay" onclick="rkCloseMenu()"></div>
<nav class="m-nav" id="m-nav" aria-label="Menu mobile">
  <div class="m-nav-head">
    <span class="wordmark"><?= htmlspecialchars($site_name) ?>.</span>
    <button class="m-close" onclick="rkCloseMenu()" aria-label="Tutup">&times;</button>
  </div>
  <?php foreach ($menus as $menu):
    $is_layanan = ($menu['url'] === '/layanan' || $menu['url'] === '/layanan/');
  ?>
    <a class="m-link" href="<?= htmlspecialchars($menu['url']) ?>"><?= htmlspecialchars($menu['label']) ?></a>
    <?php if ($is_layanan && !empty($nav_layanan)): ?>
      <div class="m-sub">
        <?php foreach ($nav_layanan as $l): ?>
          <a href="<?= url('/layanan/'.$l['slug']) ?>"><?= htmlspecialchars($l['nama']) ?></a>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
  <?php endforeach; ?>
  <a href="<?= $wa_link ?>" target="_blank" rel="noopener" class="btn red" style="margin-top:24px">
    <span class="ic"><svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor"><path d="M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.16-.17.2-.35.22-.64.08-.3-.15-1.26-.46-2.39-1.48-.88-.79-1.48-1.76-1.65-2.06-.17-.3-.02-.46.13-.6.13-.14.3-.35.45-.52.15-.18.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.61-.92-2.21-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48 0 1.46 1.07 2.88 1.21 3.07.15.2 2.1 3.2 5.08 4.49.71.3 1.26.49 1.69.62.71.23 1.36.2 1.87.12.57-.08 1.76-.72 2.01-1.41.25-.7.25-1.29.17-1.41-.07-.12-.27-.2-.57-.35M12.05 21.79h-.01a9.87 9.87 0 01-5.03-1.38l-.36-.21-3.74.98 1-3.65-.24-.37a9.86 9.86 0 01-1.51-5.26c0-5.45 4.44-9.88 9.89-9.88 2.64 0 5.12 1.03 6.99 2.9a9.83 9.83 0 012.89 6.99c0 5.45-4.44 9.88-9.89 9.88m8.41-18.3A11.82 11.82 0 0012.05 0C5.5 0 .16 5.34.16 11.89c0 2.1.55 4.14 1.59 5.95L.06 24l6.3-1.65a11.88 11.88 0 005.69 1.45h.01c6.55 0 11.89-5.34 11.89-11.89 0-3.18-1.24-6.17-3.48-8.42"/></svg></span>
    Hubungi Kami
  </a>
</nav>
